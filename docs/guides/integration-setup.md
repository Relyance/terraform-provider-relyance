...
- `secret_ref` requires `runtime_mode = "IN_HOST_BYOK"`. The runtime mode is set in the Relyance
  app. A new connection starts as `RELYANCE_HOSTED`, so create the connection first, switch it to
  InHost BYOK, then add `secret_ref`.
...
- The secret must be in the same AWS account as your InHost deployment. The scanner's IAM role
  needs `secretsmanager:GetSecretValue` on the secret. The Relyance InHost AWS Terraform module
  grants it when you add a matching ARN pattern to its `byok_secret_arn_patterns` variable.
- To clear the reference, remove `secret_ref` from the configuration.
